             ...
             <div class="absolute inset-0 flex flex-col items-center justify-center bg-black/40 z-20 backdrop-blur-[2px]">
                <div class="text-center max-w-xs space-y-4">
                  ...
                  <p class="text-indigo-200 text-sm">
                    AI will analyze waste in bulk while you move. 
                    <br><span class="text-xs opacity-70">Power-saving: Scans triggered manually on a 30s cooldown.</span>
                  </p>
                  @if (game.locationError()) {
                     <p class="text-red-400 text-xs bg-red-900/20 p-2 rounded border border-red-500/20">
                       GPS Signal Lost. Can't track route.
                     </p>
                  }
          ...
          @if (passiveSessionActive()) {
            ...
            <div class="absolute inset-0 z-20 pointer-events-none flex flex-col justify-between p-6">
               ...
               <div class="mt-20 self-center bg-black/60 backdrop-blur rounded-lg px-6 py-3 border border-indigo-500/30 flex items-center gap-4">
                  ...
                  <div class="flex flex-col items-center">
                     <button (click)="triggerManualPassiveScan()" 
                             [disabled]="isScanningBackground() || isCooldownActive()"
                             class="bg-indigo-600 hover:bg-indigo-500 disabled:bg-slate-700 disabled:text-slate-400 text-white font-bold py-2 px-4 rounded-xl shadow-lg transition-transform active:scale-95 text-xs">
                        {{ getPassiveButtonText() }}
                     </button>
                  </div>
               </div>
...
export class CameraViewComponent implements AfterViewInit, OnDestroy {
  ...
  isCooldownActive(): boolean {
    const now = Date.now();
    return now - this.lastScanTime < 30000;
  }
  ...
  getPassiveButtonText() {
    ...
    const now = Date.now();
    const timeLeft = Math.ceil((30000 - (now - this.lastScanTime)) / 1000);

    if (timeLeft > 0) return `COOLDOWN (${timeLeft}s)`;
    return 'MANUAL SCAN 📸';
  }

  triggerManualPassiveScan() {
    if (this.isScanningBackground() || this.isCooldownActive()) return;

    const currentLoc = this.game.userLocation();
    this.triggerBackgroundScan(currentLoc || null);
  }
