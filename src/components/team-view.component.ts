    ...
    <div class="h-full overflow-y-auto bg-gradient-to-b from-slate-900 via-slate-800 to-slate-900 pb-24 p-6 font-inter">
      ...
      <div class="mb-6 pt-6 flex items-center justify-between">
        ...
        <div class="flex items-center gap-3">
           ...
           <button (click)="openSettings.emit()" aria-label="Open settings" class="w-10 h-10 rounded-full bg-slate-800 border border-slate-700 flex items-center justify-center text-slate-300 hover:text-white transition-colors">
...
export class TeamViewComponent implements AfterViewInit, OnDestroy {
   ...
   totalBadgesCount = computed(() => {
      return this.game.userBadges().length;
   });
