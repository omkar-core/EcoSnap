  ...
  template: `
    <div class="h-full overflow-y-auto bg-gradient-to-b from-slate-950 to-slate-900 px-6 py-8 font-inter">
      ...
      <div class="flex items-center gap-4 mb-8">
        <button (click)="back.emit()" aria-label="Go back" class="w-10 h-10 rounded-full bg-slate-800 border border-slate-700 flex items-center justify-center text-slate-300 hover:text-white transition-colors">
      ...
      <div class="space-y-4">
        ...
        <div class="w-full p-4 bg-slate-800/50 backdrop-blur-sm border border-slate-700/50 rounded-2xl mt-4">
          ...
          <div class="flex gap-2">
            ...
            <button (click)="saveApiKey()" aria-label="Save API key" class="bg-violet-600 hover:bg-violet-500 text-white rounded-lg px-4 py-2 text-sm font-bold transition-colors">Save</button>
            @if (personalKey()) {
              <button (click)="clearApiKey()" aria-label="Clear API key" class="bg-slate-700 hover:bg-slate-600 text-white rounded-lg px-3 py-2 text-sm transition-colors">Clear</button>
            }
          </div>
        </div>
