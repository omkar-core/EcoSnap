      ...
      <div class="bg-slate-900 w-full sm:w-[500px] max-h-[95vh] flex flex-col rounded-t-3xl sm:rounded-3xl border border-slate-700 shadow-2xl animate-slide-up overflow-hidden relative">
        ...
        <div class="overflow-y-auto flex-1">
          ...
          <div class="p-6 space-y-6">
            ...
            <!-- 🧬 WASTE DNA FINGERPRINT (Animated Flip Card) -->
            @if (result().dnaFingerprint; as dna) {
               <div class="relative w-full h-48 [perspective:1000px] group cursor-pointer mb-8">
                 <div class="w-full h-full absolute transition-all duration-700 [transform-style:preserve-3d] group-hover:[transform:rotateX(180deg)]">
                    
                    <!-- DNA Front -->
                    <div class="absolute w-full h-full bg-slate-800 rounded-2xl p-5 border border-cyan-500/30 shadow-[0_0_20px_rgba(6,182,212,0.15)] [backface-visibility:hidden] flex flex-col justify-between overflow-hidden">
                       ...
                       <div class="space-y-3 mt-4 relative z-10">
                          <div>
                            <div class="flex justify-between text-[10px] font-mono text-slate-400 mb-1">
                               <span>Toxicity Heatmap</span>
                               <span [class.text-red-400]="dna.toxicityLevel > 70" [class.text-yellow-400]="dna.toxicityLevel > 30 && dna.toxicityLevel <= 70" [class.text-emerald-400]="dna.toxicityLevel <= 30">{{ dna.toxicityLevel }}%</span>
                            </div>
                            <div class="h-1.5 w-full bg-slate-900 rounded-full overflow-hidden">
                               <div class="h-full bg-gradient-to-r from-emerald-500 via-yellow-500 to-red-500" [style.width]="dna.toxicityLevel + '%'"></div>
                            </div>
                          </div>
                          <div>
                            <div class="flex justify-between text-[10px] font-mono text-slate-400 mb-1">
                               <span>Microplastic Risk</span>
                               <span [class.text-purple-400]="dna.microplasticRisk > 50">{{ dna.microplasticRisk }}%</span>
                            </div>
                            <div class="h-1.5 w-full bg-slate-900 rounded-full overflow-hidden">
                               <div class="h-full bg-purple-500" [style.width]="dna.microplasticRisk + '%'"></div>
                            </div>
                          </div>
                       </div>
                    </div>
                    ...
                    <!-- DNA Back -->
                    <div class="absolute w-full h-full bg-slate-900 rounded-2xl p-5 border border-emerald-500/30 shadow-[0_0_20px_rgba(16,185,129,0.15)] [backface-visibility:hidden] [transform:rotateX(180deg)] flex flex-col justify-center items-center text-center">
                       <h4 class="text-emerald-400 font-bold uppercase tracking-widest text-xs mb-3 border-b border-emerald-500/20 pb-2 w-full">Decomposition Timeline</h4>
                       <p class="text-slate-300 font-mono text-sm leading-relaxed mb-4">
                          {{ dna.decompositionTimeline }}
                       </p>
                       <p class="text-red-400/80 italic text-xs font-semibold">
                          "{{ dna.hauntingSentence }}"
                       </p>
                    </div>
            ...
            @if (result().upcyclingRecipe; as recipe) {
              <div class="relative overflow-hidden rounded-3xl border border-indigo-500/30 bg-gradient-to-br from-indigo-900/60 to-purple-900/60 shadow-2xl transition-all group">
                ...
                <div class="relative border-b border-indigo-500/20 bg-black/20 p-6 backdrop-blur-sm">
                  <div class="flex items-start justify-between gap-4">
                    <div>
                      <div class="mb-2 flex items-center gap-2">
                        <span class="flex h-8 w-8 items-center justify-center rounded-lg bg-indigo-500/20 text-lg shadow-inner shadow-indigo-500/10 animate-pulse">🧪</span>
                        <h3 class="text-[10px] font-black uppercase tracking-[0.2em] text-indigo-400">Mutation Lab</h3>
                      </div>
                      <h2 class="text-2xl font-black leading-tight text-white font-mono tracking-tight">{{ displayedIdea() || recipe.idea }}</h2>
                    </div>
                  ...
                  <div class="mt-4 flex flex-wrap gap-3">
                    @if (recipe.difficulty) {
                      <div class="flex items-center gap-2 rounded-full border border-indigo-500/30 bg-black/50 px-3 py-1.5 shadow-sm backdrop-blur-md">
                        <span class="text-[10px] font-bold uppercase tracking-wider text-slate-400">Complexity</span>
                    ...
                    @if (recipe.estimatedCarbonSaved) {
                      <div class="flex items-center gap-2 rounded-full border border-emerald-500/30 bg-emerald-900/20 px-3 py-1.5 shadow-sm backdrop-blur-md">
                        <span class="text-[10px] font-bold uppercase tracking-wider text-slate-400">Carbon Offset</span>
                        <span class="flex items-center gap-1 text-[10px] font-black uppercase tracking-wider text-emerald-400">
                          <svg xmlns="http://www.w3.org/2000/svg" class="h-3 w-3" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M12 2v20"/><path d="M17 5H9.5a3.5 3.5 0 0 0 0 7h5a3.5 3.5 0 0 1 0 7H6"/></svg>
                          {{ recipe.estimatedCarbonSaved }}g
                        </span>
                      </div>
                    }
                  </div>
                ...
                <div class="p-6 bg-slate-900/40 relative">
                  <!-- ASCII Blueprint Background -->
                  <div class="absolute inset-0 opacity-5 pointer-events-none font-mono text-[8px] leading-none overflow-hidden text-indigo-300 select-none whitespace-pre flex flex-wrap content-start">
                     {{ '[SYS.MUT.FUSION_MTRX] INIT => ' + recipe.idea.repeat(50) }}
                  </div>
             ...
             @if (result().upcyclingRecipe) {
               @if (!upcycleClaimed()) {
                   <button (click)="claimUpcycle.emit()" class="w-full flex flex-col items-center justify-center py-5 px-4 bg-gradient-to-r from-indigo-600 to-violet-600 hover:from-indigo-500 hover:to-violet-500 border border-indigo-400 rounded-2xl transition-all shadow-[0_4px_25px_rgba(99,102,241,0.4)] group relative overflow-hidden active:scale-95 btn-shockwave">
                     <span class="text-white font-black text-base uppercase flex items-center gap-2 tracking-wide font-mono">
                        [INITIATE_FUSION] Complete
                     </span>
                     <span class="text-indigo-100 text-xs mt-1 font-bold bg-white/10 px-2 py-0.5 rounded tracking-widest">+50 MUTATION LAB XP</span>
                   </button>
               } @else {
...
export class ScanResultComponent implements OnDestroy {
  ...
  shareRecipe(recipe: any) {
    if (navigator.share) {
      navigator.share({
        title: 'EcoSnap Mutation Lab Blueprint: ' + recipe.idea,
        text: `I just unlocked a new blueprint for ${this.result().wasteType}!\nDifficulty: ${recipe.difficulty}\nCarbon Saved: ${recipe.estimatedCarbonSaved}g\n\nFind out more on EcoSnap!`,
        url: window.location.href
      }).catch(err => this.game.showToast('Share cancelled or unavailable', 'info'));
    } else {
      // Fallback
      this.game.showToast('Link copied to clipboard', 'info');
    }
  }
}